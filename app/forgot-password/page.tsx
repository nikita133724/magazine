...
const copy = {
  RU: { cabinet: 'Личный кабинет', title: 'Восстановление пароля', email: 'Email', send: 'Получить код', sending: 'Отправляем...', codeTitle: 'Введите код', codeText: 'Мы отправили код восстановления на почту. Введите код и новый пароль.', code: '000000', newPassword: 'Новый пароль от 6 символов', save: 'Обновить пароль', saving: 'Обновляем...', login: 'Вернуться ко входу', codeLabel: 'Код восстановления' },
  KZ: { cabinet: 'Жеке кабинет', title: 'Құпиясөзді қалпына келтіру', email: 'Email', send: 'Код алу', sending: 'Жіберілуде...', codeTitle: 'Код енгізіңіз', codeText: 'Поштаңызға қалпына келтіру кодын жібердік. Кодты және жаңа құпиясөзді енгізіңіз.', code: '000000', newPassword: '6 таңбадан бастап жаңа құпиясөз', save: 'Құпиясөзді жаңарту', saving: 'Жаңартылуда...', login: 'Кіруге қайту', codeLabel: 'Қалпына келтіру коды' },
};
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-12">
      <div className="mx-auto max-w-md rounded-[2rem] bg-white p-6 shadow-sm md:p-8">
        ...
        {step === 'code' && <form onSubmit={confirm} className="mt-8 space-y-4"><p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-700">{l.codeText}</p><label className="block"><span className="mb-2 block text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{l.codeLabel}</span><input required inputMode="numeric" autoComplete="one-time-code" maxLength={CODE_LENGTH} placeholder={l.code} value={code} onChange={e => setCode(e.target.value.replace(/\D/g, '').slice(0, CODE_LENGTH))} className="w-full rounded-3xl border border-violet-200 bg-violet-50/60 px-5 py-5 text-center text-3xl font-black tracking-[0.35em] text-black outline-none transition placeholder:text-slate-300 focus:border-violet-500 focus:bg-white focus:ring-4 focus:ring-violet-100" /></label><input required type="password" minLength={6} placeholder={l.newPassword} value={password} onChange={e => setPassword(e.target.value)} className="w-full rounded-2xl border border-slate-300 px-4 py-4 outline-none focus:border-violet-500 focus:ring-4 focus:ring-violet-100" />{message && <p className="rounded-2xl bg-violet-50 p-4 text-sm font-bold text-violet-800">{message}</p>}<button disabled={loading || code.length !== CODE_LENGTH || password.length < 6} className="w-full rounded-2xl bg-black py-4 text-xs font-black uppercase tracking-widest text-white transition hover:bg-violet-800 disabled:bg-slate-400">{loading ? l.saving : l.save}</button><button type="button" disabled={loading} onClick={() => sendCode()} className="w-full rounded-2xl bg-slate-100 py-4 text-xs font-black uppercase tracking-widest text-slate-800 transition hover:bg-violet-50 disabled:opacity-50">{copy[lang].send}</button></form>}
