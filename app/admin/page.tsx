...
export default function AdminPage() {
  ...
  return (
    <AdminShell title="Панель управления">
      ...
      <div className="mt-6 rounded-[2rem] border border-violet-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-2xl font-black uppercase italic tracking-tighter">График дохода</h2>{updatedAt && <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Обновлено {updatedAt}</p>}</div><div className="mt-6 flex h-64 items-end gap-3 border-b border-slate-200 pb-2">{days.length ? days.map((x, i) => { const value = Number(x.revenue || 0); return <div key={`${x.day}-${i}`} className="flex flex-1 flex-col items-center gap-2"><div title={`${value.toLocaleString()} ₸`} className={`w-full rounded-t-2xl transition ${value > 0 ? 'bg-violet-700 hover:bg-violet-900' : 'bg-violet-100'}`} style={{ height: `${value > 0 ? Math.max(12, (value / maxRevenue) * 100) : 6}%` }} /><span className="text-[10px] font-bold text-slate-700">{x.day.slice(5)}</span></div>; }) : <p className="text-sm text-slate-700">После первого заказа здесь появится график.</p>}</div></div>
