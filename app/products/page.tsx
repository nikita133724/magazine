...
const labels = {
  RU: { title: 'Каталог', found: 'товаров', search: 'Поиск товаров', filters: 'Фильтры', all: 'Все', apparel: 'Одежда', footwear: 'Обувь', accessories: 'Аксессуары', new: 'Новинки', best: 'Хиты', sale: 'Скидки', sort: 'Сортировка', popular: 'Популярные', newest: 'Новые', low: 'Дешевле', high: 'Дороже', empty: 'Ничего не найдено', emptyText: 'Попробуйте изменить поиск или фильтры.', reset: 'Сбросить' },
  KZ: { title: 'Каталог', found: 'тауар', search: 'Тауар іздеу', filters: 'Фильтрлер', all: 'Барлығы', apparel: 'Киім', footwear: 'Аяқ киім', accessories: 'Аксессуарлар', new: 'Жаңалар', best: 'Хиттер', sale: 'Жеңілдік', sort: 'Сұрыптау', popular: 'Танымал', newest: 'Жаңа', low: 'Арзанырақ', high: 'Қымбатырақ', empty: 'Ештеңе табылмады', emptyText: 'Іздеу немесе фильтрлерді өзгертіп көріңіз.', reset: 'Тазалау' },
};
...
function ProductsContent() {
  ...
  useEffect(() => {
    ...
    fetch('/api/products', { cache: 'no-store' })
      .then(r => r.json())
      .then(d => { if (!ignore && Array.isArray(d)) setProducts(d); })
      .catch(console.error)
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      return okSearch && okCategory && okTag;
    }).sort((a, b) => {
  ...
  }, [products, search, category, tag, sort]);

  const resetFilters = () => { setSearch(''); setCategory('all'); setTag('all'); setSort('popular'); };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="mx-auto grid max-w-screen-2xl gap-8 px-4 py-8 md:px-8 lg:grid-cols-[280px_1fr]">
        <aside className="h-fit rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-28"><div className="mb-6 flex items-center gap-2 text-sm font-black uppercase tracking-widest text-slate-900"><SlidersHorizontal size={17} aria-hidden="true" /> {l.filters}</div><div className="space-y-6"><div className="flex flex-wrap gap-2">{categories.map(([value, text]) => <button key={value} onClick={() => setCategory(value)} className={`rounded-full px-4 py-2 text-xs font-black uppercase ${category === value ? 'bg-black text-white' : 'bg-slate-100 text-slate-700 hover:bg-violet-50'}`}>{text}</button>)}</div><div className="flex flex-wrap gap-2">{tags.map(([value, text]) => <button key={value} onClick={() => setTag(value)} className={`rounded-full px-4 py-2 text-xs font-black uppercase ${tag === value ? 'bg-violet-700 text-white' : 'bg-slate-100 text-slate-700 hover:bg-violet-50'}`}>{text}</button>)}</div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-700">{l.sort}<select value={sort} onChange={e => setSort(e.target.value as SortMode)} className="mt-3 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold"><option value="popular">{l.popular}</option><option value="new">{l.newest}</option><option value="price_asc">{l.low}</option><option value="price_desc">{l.high}</option></select></label></div></aside>
