...
function getCatalogMaxPrice(items: Product[]) {
  const max = Math.max(...items.map(p => Number(p.price || 0)), 0);
...
function ProductsContent() {
  ...
  const catalogMaxPrice = useMemo(() => getCatalogMaxPrice(products), [products]);
    ...
    fetch('/api/products', { cache: 'no-store' })
      ...
      .then(d => {
        if (!ignore && Array.isArray(d)) {
          setProducts(d);
          setMaxPrice(getCatalogMaxPrice(d));
        }
      })
      .catch(console.error)
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(p => {
      const text = [p.name, p.name_ru, p.name_kz, p.sub_category, p.sub_category_ru, p.sub_category_kz, p.category_name].filter(Boolean).join(' ').toLowerCase();
      ...
      return okSearch && okCategory && okTag && Number(p.price || 0) <= effectiveMaxPrice;
    }).sort((a, b) => {
      if (sort === 'new') return Number(b.is_new) - Number(a.is_new) || b.id - a.id;
      if (sort === 'price_asc') return a.price - b.price;
      if (sort === 'price_desc') return b.price - a.price;
      return Number(b.is_bestseller) - Number(a.is_bestseller) || Number(b.is_featured) - Number(a.is_featured) || Number(b.is_new) - Number(a.is_new) || b.id - a.id;
    });
  }, [products, search, category, tag, sort, effectiveMaxPrice]);

  const resetFilters = () => { setSearch(''); setCategory('all'); setTag('all'); setMaxPrice(catalogMaxPrice); setSort('popular'); };
  const priceStep = catalogMaxPrice > 1000000 ? 100000 : 10000;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="mx-auto grid max-w-screen-2xl gap-8 px-4 py-8 md:px-8 lg:grid-cols-[280px_1fr]">
        <aside className="h-fit rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-28"><div className="mb-6 flex items-center gap-2 text-sm font-black uppercase tracking-widest text-slate-900"><SlidersHorizontal size={17} aria-hidden="true" /> {l.filters}</div><div className="space-y-6"><div className="flex flex-wrap gap-2">{categories.map(([value, text]) => <button key={value} onClick={() => setCategory(value)} className={`rounded-full px-4 py-2 text-xs font-black uppercase ${category === value ? 'bg-black text-white' : 'bg-slate-100 text-slate-700 hover:bg-violet-50'}`}>{text}</button>)}</div><div className="flex flex-wrap gap-2">{tags.map(([value, text]) => <button key={value} onClick={() => setTag(value)} className={`rounded-full px-4 py-2 text-xs font-black uppercase ${tag === value ? 'bg-violet-700 text-white' : 'bg-slate-100 text-slate-700 hover:bg-violet-50'}`}>{text}</button>)}</div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-700">{l.price} {effectiveMaxPrice.toLocaleString()} ₸<input aria-label={l.price} type="range" min="0" max={catalogMaxPrice} step={priceStep} value={effectiveMaxPrice} onChange={e => setMaxPrice(Number(e.target.value))} className="mt-3 w-full accent-violet-700" /></label><label className="block text-[10px] font-black uppercase tracking-widest text-slate-700">{l.sort}<select value={sort} onChange={e => setSort(e.target.value as SortMode)} className="mt-3 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold"><option value="popular">{l.popular}</option><option value="new">{l.newest}</option><option value="price_asc">{l.low}</option><option value="price_desc">{l.high}</option></select></label></div></aside>
        <div>{loading ? <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-[430px] animate-pulse rounded-[2rem] bg-slate-100" />)}</div> : filtered.length ? <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid gap-x-5 gap-y-10 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{filtered.map(p => <ProductCard key={p.id} product={p} />)}</motion.div> : <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[2rem] border border-dashed border-slate-300 text-center"><X className="mb-4 text-slate-500" size={46} aria-hidden="true" /><h2 className="text-2xl font-black uppercase italic tracking-tighter">{l.empty}</h2><p className="mt-2 text-sm text-slate-700">{l.emptyText}</p><button onClick={resetFilters} className="mt-6 rounded-full bg-black px-6 py-3 text-xs font-black uppercase tracking-widest text-white hover:bg-violet-800">{l.reset}</button></div>}</div>
      </section>
    </div>
  );
}
