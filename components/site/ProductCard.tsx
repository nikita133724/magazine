...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group min-w-0">
      ...
      <div className="pt-4">
        <div className="mb-2 flex items-start justify-between gap-3"><Link href={productHref} className="min-w-0"><h3 className="line-clamp-2 text-base font-black uppercase italic tracking-tighter group-hover:text-violet-700">{product.name}</h3></Link><div className="flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black"><Star size={10} fill="black" />{product.rating.toFixed(1)}</div></div>
