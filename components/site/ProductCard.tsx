  ...
  return (
    <article className="group min-w-0">
      <div className="relative overflow-hidden rounded-[1.75rem] border border-slate-100 bg-slate-50 shadow-sm transition group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-violet-100">
        ...
        <button disabled={disabled} onClick={() => addToCart({ id: product.id, slug: product.slug, name: product.name, price: product.price, image, size: firstSize })} className="absolute inset-x-3 bottom-3 rounded-2xl bg-black px-4 py-3 text-[10px] font-black uppercase tracking-widest text-white opacity-0 transition hover:bg-violet-700 disabled:bg-slate-300 group-hover:opacity-100"><ShoppingBag className="mr-2 inline" size={14} />{disabled ? t('out_of_stock') : t('add_to_cart')}</button>
      </div>
      <div className="pt-4">
